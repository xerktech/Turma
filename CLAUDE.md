...
| `windows-launcher.md` | `agent/native/windows/**` | native Windows launcher (turma-agent.ps1, XERK-669) + service/control surface (turma-agent.xml, turma-agentctl.ps1, XERK-671) + installer (install.ps1, XERK-672) + self-updater (turma-agent-update.ps1, XERK-674): WinSW unit, start/stop/restart/status/logs, session-preserving restart, pidfile, the update poller + payload swap |
