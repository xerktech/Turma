...
- **PR gates (pre-merge to main).** The build workflows above run only post-merge; these run on `pull_request` → `main` (open + each push) and block the merge on findings: `code-scan.yml` (Semgrep SAST over the JS/Python + Dockerfiles + secret patterns, hadolint on both Dockerfiles, ShellCheck on `entrypoint.sh`); `agent-image-scan.yml` / `agent-hub-image-scan.yml` build each image locally (no push) and Trivy-scan it for CVEs + secrets (`ignore-unfixed`, HIGH/CRITICAL gate), path-filtered to their folder like the build workflows. SARIF is uploaded to the Security tab where GitHub Advanced Security is enabled and soft-fails otherwise, so the tool exit codes are the real gate. Because the images bundle third-party binaries (docker CLI, `gh`, ttyd, npm), keep the pinned tool versions current — that's how most CVEs are cleared; genuinely non-actionable upstream base-image findings go in the root `.trivyignore` (a reviewed triage list, each with a reason), and anything unlisted still fails the scan.
...
- The RC session runs as root (needs full host access) as an interactive `claude --remote-control` with `--permission-mode bypassPermissions`, which Claude Code refuses under root unless `IS_SANDBOX` is set — set in the compose env. (It's deliberately the interactive form, not `claude remote-control` server mode: server mode's terminal is a QR/status lobby with no conversation, so the Agent Hub's live terminal would have nothing to show or type into.)
