...
//
// Worded to match the other clients — `TurmaNav.refusalText` in
// `turma/public/nav.js` and `hubErrorMessage` in `android/.../net/HubApi.kt` —
// so the same refusal reads the same on all of them.
...
//
// A request's timeout bounds the RESPONSE, not the body that follows it, so a
// hub that sends headers and then stalls mid-body leaves this read pending
// forever — and App.poll() re-arms only in its `finally`, so ONE stalled error
// body would kill the poll loop permanently.
//
// Today's transport can't actually stall here (`proxy-fetch.ts` hands back a
// body already materialized by the background context, so `json()` is a
// synchronous parse), which is exactly why the guard belongs in the fork that
// doesn't need it: this file is held at parity with `glasses/src/hub-client.ts`,
// where the read is a live socket. A refusal body is a few bytes of JSON, so
// the ceiling can only fire on a stall.
...
// arrives freezes the display just as dead as a refused one.
//
...
// EVERY body read in this file goes through here. A request timeout bounds the
...
// `refusal`) — except getHistory's 202 ("still fetching"),
