...
- **Liveness is the AGENT PANE, not the tmux name** (XERK-1037). The runtime is the first pane's
  ...
  - **While its own tmux lives, the agent pane is alive wherever it is listed** — a cross-session
    `swap-window`/`join-pane` moves it without ending it (reaping then also killed the OTHER
    session's window). **Never on the pane id alone**: ids are unique only per tmux SERVER, which
    exits with its last session and restarts at `%0`, so a later pane would keep a dead record
    `running` forever.
