...
- **A ticket can run on the dsh OR qwen RUNTIME** (XERK-473 dsh, XERK-515 qwen): the hub's
  per-ticket runtime pin rides the command as `agentType`, which `spawn_ticket` forwards to `spawn()`
  (validated by `resolve_agent_type`, refused where that runtime is not configured). The launch
  choke point already dispatches on `agentType`, and `_launch_dsh`/`_launch_qwen` already append the
  ticket-branch directive and deliver the built ticket prompt + attachments — so a dsh/qwen ticket
  session needs NO new launch code. The hub routes each only to a host offering that runtime
  (`.claude/rules/dsh.md` [I], `.claude/rules/qwen.md` [Qwen I]).
