...
- **`qwen` is the heartbeat capability block `{available}`**, backed by `qwen_configured()` (env gate
  ...
  - **`qwen_configured()` is ALSO gated on the runtime FILES being present**
    ...
    restart. Tests: `test_qwen_configured_requires_the_runtime_files`,
    `test_qwen_runtime_present_checks_every_launch_file`.
...
- **`ui.useTerminalBuffer:false` is a FLICKER pin, not a preference.** qwen's default alt-screen
  ...
  which the tmux pane satisfies, so it turns ON and repaints the WHOLE screen per frame (`?1049` +
  `?2J` + cursor-home; append-only emits none). qwen wraps a frame in synchronized-output (`?2026`)
  to hide the repaint, but ttyd 1.7.4's xterm.js has NO mode-2026 support, so the clear+repaint
  renders progressively in the browser = flicker. Claude Code is smooth in this SAME pipeline only
  because it writes history incrementally to scrollback; `false` gives qwen that discipline. The
  trade is qwen's in-app viewport extras (in-app scroll, click-a-menu-option, hover) — xterm.js owns
  scrollback and a Claude session never had them. Host-proof only (qwen not in CI); the render-mode
  flip is measured off the real binary's escape output, not asserted in unit tests.
...
- **HITL is TWO inputs, both to full parity, KEPT not degraded:**
  ...
  - **(2) A STRUCTURED question renders the SAME multi-select card a Claude session shows** — NOT a
    yes/no approval. qwen 0.22.x DOES ship a native `ask_user_question` (G0's "no native tool" was
    WRONG, XERK-520 D2), but it renders its selector IN THE PANE and writes NO rendezvous file — so
    ...
    - **`tools.exclude` matches a tool's REGISTERED name; the built-in is the BARE name, an MCP tool
      is server-prefixed (`mcp__turma-ask__ask_user_question`)** — so excluding `ask_user_question`
      drops only the built-in, never the MCP replacement (confirmed against the 0.22.2 bundle:
      `excludeTools.includes(funcDecl.name)`; `excludeTools` is the legacy alias for `tools.exclude`).
    - **With the built-in gone the model selects the MCP tool by its DESCRIPTION, not a bare name** —
      so nothing may add a directive naming a bare `ask_user_question` (it no longer exists as a bare
      tool). The `tools.exclude` key is host-proof-only like every qwen settings key.
