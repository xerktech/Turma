...
function epicRunCompleteSweep() {
  ...
  const rows = fleetTicketRows();
  const isDone = (siteKey, key) => {
    const r = rows.get(ticketQueueKey(siteKey, key));
    return !!(r && r.row && r.row.statusCategory === "done");
  };
  for (const run of Object.values(epicRuns)) {
    if (!run) continue;
    const { siteKey, epicKey, children } = run;
    // A run with no children never auto-completes — "every child Done" is
    // vacuously true for an empty list, but an epic with no tracked children is
    // not a finished run, it is one that was armed against nothing.
    if (!Array.isArray(children) || !children.length) continue;
    if (!children.every((k) => isDone(siteKey, k))) continue;   // a wave still in flight
    const tkey = siteKey + "\x00" + epicKey;
    // Every child is Done: the epic must reach Done exactly once and the run must
    // be terminal. The write is gated on the epic NOT already being Done on the
    // board and NOT already written this lifetime — NOT on the run's `state`.
    // (B's armEpicRun sets state:"done" for a run ARMED already-complete but never
    // writes the epic; and a human may have moved the epic Done out of band — both
    // are handled by keying the write off the board, and the `state` off "did we
    // get here", so the organizer is never left In Progress.)
    if (!isDone(siteKey, epicKey) && !epicDoneWritten.has(tkey)) {
    ...
    // The epic is Done or its write is on its way — retire the run. Preserve
    // waves/children/startedAt; only state + updatedAt move (idempotent when B
    // already marked it done, so no redundant SSE frame on every later sweep).
    if (run.state !== "done") {
      run.state = "done";
