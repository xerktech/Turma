...
// Wire every externalized store onto the resolved backend at boot (never under
// TURMA_TEST, where this block does not run and `persist()` no-ops). Registers a
// watch per store, then loads the current value once the backend is ready. A key
// ABSENT from the store on first boot (a fresh Valkey, or a fresh /data) is SEEDED
// from the file/seed-primed mirror — migrating existing single-process state up to
// the shared store with no operator action.
    ...
    for (const desc of externalizedStores) {
      Promise.resolve(store.get(desc.key)).then((raw) => {
        if (raw == null) {
          ...
          if (!empty && liveStore) {
            Promise.resolve(liveStore.set(desc.key, cur)).catch((e) =>
...
function buildAgentsCache() {
  prune();
  const now = Date.now();
  // Computed ONCE for the whole payload (XERK-544/548): serializeAgent needs the
...
// --- Pausing auto-start on a maxed Claude subscription (XERK-544, XERK-548) ---
//
// Auto-start PAUSES for the hosts on a Claude subscription that has hit either of
// its limits — new tickets can still be started manually, auto mode stays
// enabled, and it RESUMES on its own as the relevant window recovers. Nothing is
// persisted: it is re-derived every read from live `limits` + `now`, which is
// exactly what makes the resume automatic. Two triggers:
//   - **7-day PACE line** (XERK-544): the consumed fraction (`sevenDay.usedPct/
//     100`) has reached the fraction of the fixed 7-day window elapsed at `now`
//     — the "on-pace" line the Usage page draws (XERK-536, until now purely
//     informational). Resumes as the window elapses (the line moves out ahead of
//     the fill) or resets.
//   - **5-hour CAP** (XERK-548): `fiveHour.usedPct` is at or past
//     `FIVE_HOUR_PAUSE_PCT` (90%). Resumes when the 5-hour window RESETS —
//     detected without waiting for the next probe by treating a `resetsAt` that
//     has passed as "already rolled over".
//
// A snapshot older than LIMIT_MAX_AGE_SEC is "can't tell", never a pause — the
// hub keeps an offline host's last heartbeat for days, so an ancient
// `capturedAt` is not a fact about now. Mirrors usage.html's LIMIT_MAX_AGE_SEC
// (the age at which it drops a limit card entirely). Reached only from
// request-time helpers, never the hoisted restore loop, so a module const is
// safe here.
const LIMIT_MAX_AGE_SEC = 24 * 60 * 60;
const SEVEN_DAY_WINDOW_SEC = 7 * 86400;
// The 5-hour window used-% at or above which auto-start pauses (XERK-548). The
// ticket's line, "at or past 90%", so the compare is `>=`.
const FIVE_HOUR_PAUSE_PCT = 90;
...
// Is this snapshot's 5-hour window at or past the pause cap (XERK-548)? Stale/
// absent/typeless all read as NOT maxed ("can't tell" never pauses). A window
// whose `resetsAt` has PASSED has since rolled over — the used-% is a stale
// high-water mark for a window that no longer exists, so it reads not-maxed and
// the host RESUMES the instant the window resets, without waiting for the next
// probe to report a fresh low reading (the auto-resume the ticket asks for).
function limitsFiveHourMaxed(limits, nowMs) {
  if (!limits || typeof limits !== "object") return false;
  const nowSec = Math.floor(nowMs / 1000);
  if (nowSec - (limits.capturedAt || 0) > LIMIT_MAX_AGE_SEC) return false;
  const win = limits.fiveHour;
  if (!win || typeof win.usedPct !== "number" || typeof win.resetsAt !== "number") return false;
  if (win.resetsAt <= nowSec) return false;   // window already reset → resumed
  return win.usedPct >= FIVE_HOUR_PAUSE_PCT;
}

// Does this subscription snapshot pause auto-start, for EITHER reason? The one
// predicate the per-subscription set and the tests go through, so the two
// triggers can never drift apart.
function subscriptionLimitsPaused(limits, nowMs) {
  return limitsPastPace(limits, nowMs) || limitsFiveHourMaxed(limits, nowMs);
}
...
// The set of subscription keys a limit pauses (XERK-544 pace line OR XERK-548
// 5-hour cap). Grouped like the Usage page's limitGroups (by subscription key),
// so two hosts sharing one Claude account judge against ONE pool and the
...
// whose EFFECTIVE default runtime is claude (an absent field already coerced to
// "claude") spends the Claude pool for an unpinned auto ticket — a qwen/dsh
// default host spends none, so it NEVER pauses however maxed the account is.
// Claude-only by construction: dsh/qwen have no subscription window, so their
// spend never reaches `limits` (agent-usage.md). `paused` is the set from
// pausedSubscriptions, computed once by the caller.
function hostAutoPaused(key, a, paused) {
  if ((a.defaultRuntime || "claude") !== "claude") return false;
  return paused.has(subscriptionKeyOf(key, a));
}
...
function readBody(req, cap = BODY_MAX, costPerByte = BODY_PARSE_COST) {
  return new Promise((resolve, reject) => {
    ...
    // A drainable SIZE refusal holds its BodyTooLarge here and defers the reject
    // (and thus the route's 413) until the body has finished arriving — see the
    // refuse() comment below. Budget/no-drain refusals reject at once.
    let pendingTooLarge = null;
    const refuse = (err, from = cap) => {
      ...
      // agents post with); answering mid-stream and then closing the connection
      // reaches it as a bare ECONNRESET, not the 413 it must resize against —
      // the very socket-hang-up draining was added to prevent (XERK-235),
      // resurfacing as a race under CPU load (XERK-291). Draining to `end` first
      // means the client has finished writing before the status goes out and no
    ...
    req.on("data", (c) => {
      len += c.length;
      if (over) {
        // Past the drain window the courtesy is what is killing the hub: give up
...
// How much past `cap` the body readers keep draining before they give up on
// saying anything and cut the socket. Draining is what lets a refused body's
// 413 reach the client on the same connection instead of a socket hang up, and
// "the network broke" is the wrong thing to tell someone whose file was simply
// too big (XERK-234). An honest overshoot within this window drains to `end` and
// the 413 is answered there, cleanly (XERK-291); only a body PAST it is cut. So
// it must clear a realistic honest overshoot — an agent whose one beat outgrew
// HEARTBEAT_MAX and must read the 413 to resize — with margin over the 1 MiB
// that left the repro landing exactly on the old boundary. Raising it only lets
// each of at most DRAIN_CONCURRENCY_MAX drains read a little further before the
// cut; the bytes are discarded as they arrive, never buffered.
const RAW_BODY_DRAIN_SLACK = 4 << 20; // 4 MiB
...
function readRawBody(req, cap) {
  return new Promise((resolve, reject) => {
    ...
    req.on("close", () => { endDrain(); disarmIdle(); });
    // Mirrors readBody: a drainable SIZE refusal defers its reject to `end`.
    let pendingTooLarge = null;
    const refuse = (err, from = cap) => {
      over = true;
...
// XERK-705: a merged PR does NOT close the ticket or kill the session any more, for
// EITHER stream (the org auto-merge stream AND epic-run children). The session may
// still have work to do (the PR was just one step), so the hub MESSAGES the running
// session to mark its OWN ticket Done once it judges the work complete; that Done edge
// then auto-STOPS it (XERK-45) and, for an epic child, advances the wave / completes
// the epic (XERK-636/637). This map remembers, per session, which merged PR urls it
// has already been told about, so a FOLLOW-UP PR merging re-fires the message but a
// repeat sweep over the same set does not. Keyed "<host>\x00<sessionId>" ->
// {at, urls:Set<string>}; bounded oldest-first. In-memory only (like autoStopped): a
...
// MERGED, so a purely-CLOSED PR never closes its ticket), the hub MESSAGES the
// running session to mark ITS OWN ticket Done — it never writes Done or kills the
// session itself (XERK-705). A merged PR is one step, not proof the whole ticket is
// finished (the session may have a follow-up PR, tests, a migration), so each ticket
// is TASKED TO SELF-CLOSE: the session validates its work and moves the ticket to
// Done. That Done edge then does everything the hub used to force:
//   - autoStopSweep (XERK-45, opt-in-independent) kills the session and frees the slot;
//   - for an epic-run child, the driver (epicRunDriveSweep, XERK-636) re-evaluates
//     readiness off the child's Done and starts the next wave, and once every child is
//     Done epicRunCompleteSweep (XERK-637) writes the EPIC to Done.
//
// This applies to BOTH eligibility streams identically — the ORG auto-merge stream
// (autoMergeSession) AND epic-run children (epicRunChildSession). An epic run no
// longer force-closes its children (the earlier XERK-637 direct Done+kill is gone):
// arming a run is still the opt-in that gets the child auto-MERGED and messaged, but
// the child, like any session, decides for itself when its work is done. TRADE-OFF: a
// child (or org-stream session) that neither continues nor self-marks Done holds its
// slot and, for an epic, stalls the wave — the deliberate cost of not closing a ticket
// behind live work.
//
// The message re-fires per NEW merged PR (autoCloseNotified per session), never on a
// repeat sweep over the same merged set.
function autoCloseSweep() {
  ...
  for (const [host, a] of Object.entries(agents)) {
    for (const s of a.sessions || []) {
      ...
      // Message the session; never close/kill it here. Fire only when a merged PR we
...
// ITSELF to Done (the XERK-138 status write-back, routed to a board-cred host of the
// org) and mark the run terminal. The epic is an organizer with no session of its
// own, so the hub still writes its Done directly (there is nothing to message).
//
...
// the child's self-close Done edge (XERK-705) or a human move is what C re-evaluates
// against.
...
if (process.env.TURMA_TEST) {
  module.exports = {
    ...
    // per-subscription set directly, not only through a full findTicketHost/sweep.
    sevenDayPaceFrac,
    limitsPastPace,
    limitsFiveHourMaxed,
    subscriptionLimitsPaused,
    pausedSubscriptions,
    hostAutoPaused,
    FIVE_HOUR_PAUSE_PCT,
    LIMIT_MAX_AGE_SEC,
  ...
  // file/in-memory backend (its `persistent` map reproduces each store's on-disk
  // JSON byte-for-byte); with HA on it is the Valkey client, whose connection
  // health is logged as it comes and goes — never fatal at runtime, so a store
  // blip reconnects rather than taking the hub down (availability). The file
  // backend's `persistent` config is the same /data files the stores used to
  // write themselves, so nothing on disk moves.
  const backendStore = createLiveStore(haConfig, {
