...
// SKIPPED AS ROOT, not fixed differently: root ignores directory permission bits,
// so `chmod 0555` does not stop the rename and the case's whole premise is absent —
// the hub renames the file successfully and correctly says so. CI runs as a normal
// user and exercises this for real; a container run as root (how the suite is run
// locally on the fleet's own hosts) would otherwise report a failure that says
// nothing about the code. Not part of XERK-258; found because that ticket's own
// green-on-CI-red-locally test taught me to run the suites both ways.
test("when it CANNOT move the file, it says that instead of naming one that isn't there", {
  skip: typeof process.getuid === "function" && process.getuid() === 0
    ? "root bypasses the read-only directory this case depends on"
    : false,
}, () => {
