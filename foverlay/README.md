# Turma — Foverlay miniapp

The Foverlay bundled-miniapp build of the Turma glasses client: the same Even
Realities G2 experience as `glasses/` (session list, live transcript, question
answering, G2-mic dictation), running as a Mentra miniapp inside the Foverlay
app instead of as an Even Hub plugin. Ported from `glasses/` in Foverlay's
...
Foverlay monorepo, since neither is published to a public registry:
...
To regenerate after an SDK change: in the Foverlay monorepo run `bun pm pack`
...
`background/` and `ui/` — that is the shape the Foverlay app expects for a
bundled miniapp. `pack.mjs` accepts `--version X.Y.Z` and `--out <path>`.

## Release flow

`release.yml`'s `build-foverlay` job stamps the unified release version into
`miniapp.json`, builds, and packs to a `turma-foverlay-v<version>.zip` GitHub
