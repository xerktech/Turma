...
# mounted. This writes one from the projected token so that credential has a
# named context and a default namespace.
#
# BE HONEST ABOUT WHAT THIS BUYS, because it decides how hard the block may try.
# client-go already falls back to the in-cluster credential on its own: with no
# /root/.kube/config at all, `kubectl` and `helm` in a pod still authenticate as
# the ServiceAccount. What this adds is a config an operator can read, a
# current-context, and the pod's own namespace as the default instead of
# `default`. That is worth having and worth NOTHING at the price of a boot: this
# is PID 1, and every other preflight in this file is log-only for that reason.
# So every step below is guarded and every failure is a log line — QA measured
# an unguarded `mkdir` killing the container outright on a read-only rootfs and
# on a /root/.kube that was a file, and an unbounded read of the namespace file
# hanging PID 1 forever on a fifo.
#
# `tokenFile:` rather than an inline `token:` is what makes rotation work. A
# projected token is re-written in place at ~80% of its TTL; QA measured a copy
...
# config pairs with the pod's real bearer token. **What stops that token
# reaching an attacker's server is the CA pin, and only the CA pin.**
# `certificate-authority` is the cluster's own CA out of the ServiceAccount
# directory, so a redirected apiserver cannot complete the TLS handshake and no
# HTTP request is ever made — QA measured a redirected kubectl failing with
# `x509: certificate signed by unknown authority` and the rogue server logging a
# `bad certificate` alert with ZERO requests received.
#
# So never drop `certificate-authority`, never add `insecure-skip-tls-verify`,
# and never reuse this env for anything that is not CA-pinned.
#
# Do NOT restore the reasoning this replaced ("anything that can set env on a
# container can do worse directly"). It is false for this variable: a Service
# named `kubernetes` in the pod's own namespace overrides
# `KUBERNETES_SERVICE_HOST` for every pod in it, and namespace-scoped `create
# services` is far less privilege than editing a workload. Measured on k8x.
# Writing the file also adds no trust that not writing it would avoid —
# client-go's in-cluster fallback reads the same variable with the same CA.
#
...
# persistent volume in a pod, so without it the first boot's config would be
# frozen there forever and a changed apiserver or namespace would lose silently.
KUBE_SA_DIR=/var/run/secrets/kubernetes.io/serviceaccount
KUBE_GEN_MARK="# generated by turma entrypoint.sh from this pod's ServiceAccount"
# Where the config is staged before it is moved into place. Ours by NAME only —
# do not read this as "nothing else writes here", and do not let a change to the
# `chown` further down persuade you otherwise. What re-owns this directory is
# the IDENTITY SELF-HEAL near the top of this file, which chowns every root-owned
# path under /root/.kube (and ~/.aws, ~/.talos, …) to the run-as identity on
# every drop-priv boot, whether this block runs or not — and /root itself is
# chowned there too, so a session can replace /root/.kube wholesale. Every line
# below assumes exactly that.
KUBE_TMPDIR=/root/.kube/.turma-tmp
KUBE_FROM_SA=no

# Is /root/.kube/config ours to overwrite? Ours, absent, or a 0-byte residue of
# a half-finished write all count as yes; anything else is the operator's.
kube_config_is_ours() {
  # A symlink — dangling included — is somebody's deliberate redirect, and a
  # `cat >` would follow it and create the target. Checked FIRST, because a
  # dangling one fails `-e` and would otherwise read as "absent".
...
if [ -z "$KUBE_CLI" ]; then
  ...
  KUBE_FROM_SA=skip
     ...
     && [ -r "${KUBE_SA_DIR}/token" ] \
     && [ -r "${KUBE_SA_DIR}/ca.crt" ] \
     && kube_config_is_ours; then
  # Everything interpolated into the heredoc is validated first. These sources
  # are the kubelet's today, so this is robustness rather than a threat model —
  # but a namespace file with a stray newline, or a port that isn't one, would
  # otherwise produce a YAML file that parses as something else entirely.
  KUBE_API_HOST="${KUBERNETES_SERVICE_HOST}"
  case "$KUBE_API_HOST" in
    *[!0-9a-zA-Z.:-]*|'') KUBE_API_HOST="" ;;
    # An IPv6 apiserver address has to be bracketed in the URL. This cluster is
    # v4-only, so the branch is untested there and costs one line to be right.
    *:*) KUBE_API_HOST="[${KUBE_API_HOST}]" ;;
  esac
  KUBE_API_PORT="${KUBERNETES_SERVICE_PORT:-443}"
  case "$KUBE_API_PORT" in
    *[!0-9]*|'') KUBE_API_PORT=443 ;;
  esac
  # Bounded, because this path can be a fifo — a plain `cat` there never
  # returns and PID 1 sits in it forever with the container reported up.
  KUBE_NS="$(timeout 2 cat "${KUBE_SA_DIR}/namespace" 2>/dev/null || true)"
  case "$KUBE_NS" in
    *[!a-z0-9-]*|'') KUBE_NS=default ;;
  esac
  [ "${#KUBE_NS}" -le 63 ] || KUBE_NS=default

  if [ -L /root/.kube ]; then
    # THE DIRECTORY ITSELF CAN BE A SESSION'S SYMLINK. `/root` is handed to the
    # run-as identity by the identity block above, so a session can replace
    # /root/.kube wholesale — and then everything below happens inside a
    # directory it chose: the `rm -rf` deletes that directory's staging path and
    # its contents as root, and the config lands there root-owned. QA measured
    # both. `kube_config_is_ours` refuses a symlinked `config` one level down;
    # this is the same rule one level up, and it has to come first because
    # `mkdir -p` succeeds on a symlink to a directory.
    #
    # Refuse rather than clear it: unlike `.turma-tmp`, which is our own name,
    # this one is plausibly an operator's deliberate redirect.
  ...
  # A SUBDIRECTORY OF OUR OWN, replaced whole on every boot rather than a temp
  # file beside the operator's things. A unique `mktemp` name is not
  # self-limiting the way the fixed `config.new` it replaced was — a crash
  # between the `mktemp` and the `mv` leaves one behind on what is a persistent
  # volume in a pod — so the leftovers have to be cleared, and HOW is the whole
  # difficulty. Two earlier shapes were both worse than the problem:
  #   * `rm -f /root/.kube/.config.??????` deleted any of the operator's files
  #     whose name fit that glob (QA destroyed `.config.backup`);
  #   * `mkdir -p` + `rm -f "$KUBE_TMPDIR"/*` followed a SYMLINK planted at
  #     `.turma-tmp` and emptied whatever directory it pointed at, as root, with
  #     the boot still reporting success — and the planter needs no privilege.
  #
  # `rm -rf` ON THE PATH ITSELF is the primitive that is safe against all of it:
  # given a symlink it unlinks the LINK and never traverses to the target, given
  # a regular file it removes the file, and given our own directory it removes
  # the leftovers a glob cannot (a stray SUBdirectory made `rm -f dir/*` fail,
  # which disabled generation on every later boot). Then a bare `mkdir` — NOT
  # `-p` — refuses if anything raced back into the path, which is what closes
  # the gap between the two.
  elif ! { rm -rf "$KUBE_TMPDIR" 2>/dev/null \
    ...
    if mv -f "$KUBE_TMP" /root/.kube/config 2>/dev/null; then
      # Empty now, and an empty hidden directory in the operator's ~/.kube is
      # still clutter we put there. `rm -rf`, not `rmdir`, only so that every
      # exit from this construct clears up the same way. The next boot
      # re-creates it.
      rm -rf "$KUBE_TMPDIR" 2>/dev/null || true
...
# agent can act on it.
#
# kubectl is reported here rather than through `cloud_creds` because it has
# three states, not two, and because an EMPTY /root/.kube/config is the residue
# of a half-finished write rather than a credential — `cloud_creds` keys on
# existence, which would report that one as mounted forever.
if [ -n "$KUBE_CLI" ] && [ "$KUBE_FROM_SA" = "no" ]; then
  if [ -s /root/.kube/config ]; then
    echo "[entrypoint] ${KUBE_CLI}: host creds mounted at /root/.kube"
  else
    echo "[entrypoint] ${KUBE_CLI}: installed; no creds on this device (/root/.kube) — ignoring"
  fi
fi
