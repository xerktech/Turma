...
# do not read this as "nothing else writes here". /root/.kube is handed to the
# session identity by the `chown -R` further down, so a session can create
# anything it likes at this path; the code that uses it assumes exactly that.
...
if [ -z "$KUBE_CLI" ]; then
  ...
  #     because the `chown -R` below hands /root/.kube to the session identity.
  #
