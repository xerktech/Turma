...
# Ours, exclusively. Nothing else writes here, which is what makes sweeping it
# whole safe — see the comment at the `mkdir` below.
...
if [ -z "$KUBE_CLI" ]; then
  ...
  # A SUBDIRECTORY OF OUR OWN, created and swept whole, rather than a temp file
  # beside the operator's things. A unique `mktemp` name is not self-limiting
  # the way the fixed `config.new` it replaced was — a crash between the
  # `mktemp` and the `mv` leaves one behind on what is a persistent volume in a
  # pod — but sweeping it with a glob over /root/.kube deletes anything whose
  # name happens to fit: QA destroyed a planted `.config.backup` and
  # `.config.2026-0` that way, which is a WIDER hazard than the single fixed
  # name the whole change set out to stop clobbering. Owning the directory is
  # the only version of this that cannot guess wrong, and it is why the happy
  # path `rmdir`s it below: on a clean boot we leave nothing at all behind.
  elif ! { mkdir -p "$KUBE_TMPDIR" 2>/dev/null \
           && rm -f "$KUBE_TMPDIR"/* 2>/dev/null \
    ...
    rmdir "$KUBE_TMPDIR" 2>/dev/null || true
    echo "[entrypoint] ${KUBE_CLI}: cannot write /root/.kube/config — leaving the credential to client-go's own in-cluster fallback"
  else
    ...
    if mv -f "$KUBE_TMP" /root/.kube/config 2>/dev/null; then
      ...
      # still clutter we put there. The next boot re-creates it.
      rmdir "$KUBE_TMPDIR" 2>/dev/null || true
