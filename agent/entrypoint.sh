...
if command -v kubectl >/dev/null 2>&1 && [ "$KUBE_FROM_SA" = "no" ]; then
  if [ -s /root/.kube/config ]; then
    echo "[entrypoint] kubectl: host creds mounted at /root/.kube"
  else
    echo "[entrypoint] kubectl: installed; no creds on this device (/root/.kube) — ignoring"
  fi
fi
